...
<body>
	<div class="easyui-panel" data-options="fit:true,title:'TUP${index}配置'">
		<form id="ff" action="${pageContext.request.contextPath}/out/tup/update.action" method="post">
				...
				<tr>
					...
					<td>
						<select class="easyui-combobox" name="isup_sio" data-options="required:true,panelHeight:'auto'">
						    <option value="0x05" <c:if test="${isup.isup_sio=='0x05'}">selected</c:if>>INTERNATIONAL</option>   
						    <option value="0x84" <c:if test="${isup.isup_sio=='0x84'}">selected</c:if>>NATIONAL</option>
						</select>
					</td>
				</tr>
				<tr>
